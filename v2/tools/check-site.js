...
for (const page of ["index.html", "404.html"]) {
  ...
  if (/<script(?![^>]*\bsrc=)/i.test(source)) errors.push(`${page} contains inline JavaScript`);
  if (/\son\w+=["']/i.test(source)) errors.push(`${page} contains an inline event handler`);
}
...
const routes = [
  ...
  "public-thread", "public-new-thread", "public-delete-thread", "public-chat",
  "admin-chat", "admin-contacts", "admin-save-contacts",
  "admin-public-conversations", "admin-public-conversation",
  "admin-delete-public-conversation", "admin-block-user", "admin-unblock-user",
  "admin-context-status", "admin-context-import", "admin-context-clear"
];
...
for (const marker of [
  ...
  "Managed private pack",
  "cannot"
]) {
  if (!appSource.includes(marker)) errors.push(`app.js is missing the context manager marker: ${marker}`);
}
